...
export const useItems = (): IUseItemsResult => {
  ...
  useEffect(() => {
    ...
    getItems({ signal: controller.signal })
      ...
      .catch((err: unknown) => {
        ...
        setItems([]);
        setError(err instanceof Error ? err.message : "Не удалось получить предметы игрока.");
      })
      .finally(() => {
  ...
  const equip = async (item: IInventoryItem) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : "Не удалось надеть предмет.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const unequip = async (item: IInventoryItem) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : "Не удалось снять предмет.");
    } finally {
      setBusy(false);
    }
  };
