...
export const getExits = async ({
  ...
  if (!placeRes.ok)
    throw new Error(
      await readApiError(placeRes, "Не удалось загрузить позицию игрока."),
    )
  if (!locsRes.ok)
    throw new Error(
      await readApiError(locsRes, "Не удалось загрузить локации."),
    )
  ...
  for (const loc of locs) {
    if (loc.parentId === hereId)
      exits.push({ id: loc.id, name: loc.name, kind: "child" })
  }
