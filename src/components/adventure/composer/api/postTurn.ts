...
export type ITurnResult =
  | { status: "need_check"; replies: ITurnReply[]; check: IResolvedCheck; resume: ITurnResume }
...
export const postTurn = async ({ messages, resume, check, rollId }: IPostTurnParams): Promise<ITurnResult> => {
  const { baseUrl, campaignId, playerId } = getTurnApiEnv();
  const url = new URL("/api/turn", baseUrl);
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ campaignId, playerId, messages, resume, check, rollId }),
  });
