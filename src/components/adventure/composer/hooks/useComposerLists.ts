...
export const useComposerLists = ({ locationEpoch }: IParams) => {
  ...
  useEffect(() => {
    ...
    Promise.all([getNpcs({ signal: controller.signal }), getItems({ signal: controller.signal })])
      ...
      .catch((err: unknown) => {
        ...
        setError(err instanceof Error ? err.message : "Не удалось загрузить списки для действий.");
      })
      .finally(() => {
