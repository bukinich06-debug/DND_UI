...
export const Composer = ({ onSend, sending, error, locationEpoch }: IComposerProps) => {
  const t = useTranslations("center");
  const { npcs, items, error: listsError } = useComposerLists({ locationEpoch });
  const { action, selectAction, input, setInput, form, patchForm, canSend, send, setForm } = useComposer({
    onSend,
    sending,
    npcs,
    items,
  });
  const { exits, loading: exitsLoading, error: exitsError } = useExits(action === "move", locationEpoch);
  ...
  return (
    <div className="shrink-0 border-t border-border bg-background px-7 pt-4 pb-5">
      ...
      {listsError && <p className="mt-1.5 mb-0 font-sans text-xs text-combat">{listsError}</p>}
      {exitsError && <p className="mt-1.5 mb-0 font-sans text-xs text-combat">{exitsError}</p>}
