...
export const CharacterPanel = ({ onInventory, activeTab, setActiveTab }: ICharacterPanelProps) => {
  ...
  const classLabel = player.subclass ? `${player.className} (${player.subclass})` : player.className;
  ...
  if (player.xp !== null)
    xpBar = <BarStat label={t("xp")} current={player.xp} max={xpToNext(player.level)} color="#5a7fc9" />;
  ...
  return (
    <aside className="flex h-full w-[280px] shrink-0 flex-col border-r border-border bg-background">
      ...
      <div className="flex flex-col gap-3 border-b border-border px-4 py-3.5">
        ...
        <div className="mt-1">
          <div className="mb-1 font-sans text-xs uppercase tracking-widest text-muted">{t("purse")}</div>
          <Purse className="font-mono text-[13px] text-foreground-dim" />
        </div>
      </div>

      <div className="border-b border-border px-4 py-3.5">
        <div className="mb-2 font-sans text-xs uppercase tracking-widest text-muted">{t("abilityScores")}</div>
      ...
      <div className="flex flex-col gap-2 border-b border-border px-4 py-3.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[13px] text-muted">{t("equipped.longbowSub")}</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-gold">
          ...
          <div>
            ...
            <div className="text-[13px] text-muted">{t("equipped.leatherArmorSub")}</div>
          </div>
        </div>
      ...
      <div className="border-b border-border px-4 py-2.5">
        <div className="mb-1.5 font-sans text-xs uppercase tracking-widest text-muted">{t("conditions")}</div>
        {conditions}
      </div>
