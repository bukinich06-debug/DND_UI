...
export const usePurseState = (): IPurseContextValue => {
  ...
  const refresh = async () => {
    try {
      ...
      const nextDelta = prevCp === null ? 0 : data.coinsCp - prevCp;

      setPurse(data.coins);
      setCoinsCp(data.coinsCp);

      if (nextDelta === 0 || !prevPurse) return clearFlash();
...
export const PurseContextProvider = ({ children, value }: IPurseProviderProps) => (
